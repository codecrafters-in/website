...
export default function Home() {
  ...
  return (
    <>
      <SEO
        title="Odoo ERP & AI Automation — Ahmedabad"
        description="Senior Odoo developer (v14–v19) and AI engineer in Ahmedabad. Custom ERP implementation, LLM pipelines, AI agents. 30+ projects delivered. Book a free audit."
        keywords="Odoo developer India, Odoo ERP implementation Ahmedabad, AI automation developer, LLM development India, Odoo customisation"
        path="/"
      />
      <main className="pt-20">
        ...
        <section className="bg-surface-container-low py-24 px-5 sm:px-8 lg:px-10 xl:px-14">
          ...
          <div className="overflow-hidden -mx-5 sm:-mx-8 lg:-mx-10 xl:-mx-14 mt-10">
            <div className="marquee-track flex gap-5 w-max py-1">
              {[
                { icon: 'settings_slow_motion', title: 'Manual Work Killing Margins', body: "Your team spends 30% of their day on tasks a well-configured AI could handle in seconds. That's salary burned on copy-paste work.", cost: '~$180K/yr wasted per 10 employees' },
                { icon: 'database_off', title: 'Data Trapped in Silos', body: "Finance can't see ops. Sales can't see inventory. You're making six-figure decisions with incomplete information because your tools don't talk.", cost: 'Average 23% revenue leakage' },
                { icon: 'security_update_warning', title: "Tech That Won't Scale", body: "Every new customer or SKU adds friction. Your setup was built for where you were, not where you're going. It will break before you hit your goals.", cost: 'Growth capped at current infra' },
                { icon: 'receipt_long', title: 'Invoicing & Billing Chaos', body: "Manual invoice creation, missed follow-ups, and no reconciliation mean cash flow gaps you shouldn't have. Money owed sits uncollected for weeks.", cost: 'Avg 14-day payment delay' },
                { icon: 'manage_history', title: 'ERP Nobody Actually Uses', body: "You bought an ERP. Half your team still uses spreadsheets. The system wasn't configured for how your business actually works — so people worked around it.", cost: 'Full licence cost, zero ROI' },
                { icon: 'person_off', title: 'Key-Person Dependency', body: "Critical processes live in one person's head. When they're sick, on leave, or gone — operations stall. No documentation, no system, no safety net.", cost: 'Single point of failure risk' },
                { icon: 'monitoring', title: 'No Real-Time Visibility', body: "You find out about problems after they've already cost you. No live dashboards, no alerts, no early warning system — just spreadsheets updated once a week.", cost: 'Decisions made on stale data' },
                { icon: 'support_agent', title: 'Support Tickets Piling Up', body: "Customer queries, internal requests, vendor issues — all handled manually, dropping through the cracks. No routing, no SLAs, no accountability.", cost: 'Customer churn from slow response' },
                { icon: 'settings_slow_motion', title: 'Manual Work Killing Margins', body: "Your team spends 30% of their day on tasks a well-configured AI could handle in seconds. That's salary burned on copy-paste work.", cost: '~$180K/yr wasted per 10 employees' },
                { icon: 'database_off', title: 'Data Trapped in Silos', body: "Finance can't see ops. Sales can't see inventory. You're making six-figure decisions with incomplete information because your tools don't talk.", cost: 'Average 23% revenue leakage' },
                { icon: 'security_update_warning', title: "Tech That Won't Scale", body: "Every new customer or SKU adds friction. Your setup was built for where you were, not where you're going. It will break before you hit your goals.", cost: 'Growth capped at current infra' },
                { icon: 'receipt_long', title: 'Invoicing & Billing Chaos', body: "Manual invoice creation, missed follow-ups, and no reconciliation mean cash flow gaps you shouldn't have. Money owed sits uncollected for weeks.", cost: 'Avg 14-day payment delay' },
                ...
                <div key={i} className="w-[280px] sm:w-[300px] shrink-0 p-7 bg-surface-container-lowest border border-[#4e4633]/10 hover:border-[#f5c518]/30 transition-colors duration-300 flex flex-col">
                  <span className="material-symbols-outlined text-primary-container text-4xl mb-4 block">{card.icon}</span>
                  <h4 className="text-on-surface font-bold text-base mb-3 uppercase tracking-tight leading-snug">{card.title}</h4>
          ...
          <InView className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-10 xl:px-14">
            <div className="h-0" />
        ...
        <section className="py-24 bg-surface-container-low">
          {/* Heading + bio */}
          <InView className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-10 xl:px-14 mb-14">
            <motion.div variants={fadeUp} className="text-center mb-10">
              <p className="text-primary-container text-xs uppercase tracking-[0.3em] mb-3">Built by Jaimin Shah</p>
              <h2 className="text-3xl md:text-4xl font-black tracking-tighter text-on-surface mb-4">Results That Speak for Themselves.</h2>
              <p className="text-on-surface-variant text-base max-w-2xl mx-auto leading-relaxed">
                Senior Odoo architect and AI engineer with 4+ years shipping production systems across healthcare, fintech, logistics, and manufacturing. Published ML researcher. Every project below is real, built from scratch, and running in production.
              </p>
            </motion.div>
            {/* Stats strip */}
            <motion.div variants={stagger} className="grid grid-cols-2 md:grid-cols-4 gap-0 divide-y md:divide-y-0 md:divide-x divide-[#4e4633]/15 border border-[#4e4633]/15">
              {[['30+', 'Projects Delivered'], ['Odoo v12–v19', 'All Versions Mastered'], ['5+', 'AI Systems in Production'], ['2', 'Peer-Reviewed Publications']].map(([v, l]) => (
                <motion.div key={l} variants={fadeUp} className="py-6 px-8 text-center">
          ...
          <div className="overflow-hidden mb-5">
            <div className="marquee-track flex gap-5 w-max py-1">
              {[
                ...
                { cat: 'AI Productivity', icon: 'mic', title: 'AI Meeting Intelligence', outcome: 'Audio capture → AI summarisation → TODOs, mind-maps, conversational Q&A over transcripts, searchable knowledge base.', tags: ['LLM', 'Audio AI', 'React', 'Flask'] },
                { cat: 'AI Vision', icon: 'visibility', title: 'AI Real-Time Screen Assistant', outcome: 'Live screen analysis via Gemini Vision with voice interaction, session memory, and low-latency streaming.', tags: ['Gemini Vision', 'Voice AI', 'Python'] },
                { cat: 'Odoo', icon: 'document_scanner', title: 'Custom OCR Engine for Odoo', outcome: 'Automated invoice processing — intelligent field matching, confidence scoring, human-in-the-loop for low-confidence extractions.', tags: ['OCR', 'Odoo', 'Python', 'ML'] },
                { cat: 'eCommerce', icon: 'storefront', title: 'Multi-Vendor eCommerce Platform', outcome: 'Product configurators, multi-vendor support, GoCardless and Stripe payments, custom OWL + Bootstrap themes.', tags: ['Odoo', 'OWL', 'GoCardless', 'Stripe'] },
                { cat: 'ERP Migration', icon: 'upgrade', title: 'Odoo v14 → v19 Migration', outcome: 'Full-stack migration rewriting business logic, modernising OWL components and replacing deprecated APIs across accounting, helpdesk, and eCommerce.', tags: ['Odoo v14–v19', 'OWL', 'PostgreSQL'] },
                { cat: 'AI + CRM', icon: 'smart_toy', title: 'AI Customer Engagement Module', outcome: 'Analyses customer conversations, generates response suggestions, and schedules appointments tied to Odoo Calendar availability.', tags: ['LLM', 'Odoo CRM', 'Python'] },
                ...
                <div key={i} className="w-[300px] sm:w-[320px] shrink-0 p-6 bg-[#131313] border border-[#4e4633]/15 hover:border-[#f5c518]/40 transition-colors duration-300 flex flex-col gap-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="material-symbols-outlined text-[#4e4633]/50 text-xl">{p.icon}</span>
                  </div>
                  <h4 className="font-bold text-on-surface text-sm leading-snug">{p.title}</h4>
                  <p className="text-on-surface-variant text-xs leading-relaxed flex-grow">{p.outcome}</p>
                  <div className="flex flex-wrap gap-1.5 pt-2 border-t border-[#4e4633]/15">
                    {p.tags.map((t) => <span key={t} className="text-[9px] font-bold uppercase tracking-wider text-[#4e4633] px-2 py-0.5 border border-[#4e4633]/30">{t}</span>)}
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Row 2 — Testimonials scrolling right */}
          <div className="overflow-hidden">
            <div className="marquee-track-reverse flex gap-5 w-max py-1">
              {[
                { quote: 'CodeCrafters reduced our order processing time from 4 hours to 8 minutes. The Odoo implementation paid for itself in the first quarter.', name: 'S.C.*', title: 'COO, Logistics Enterprise', metric: '96% faster processing' },
                { quote: 'Their AI pipeline replaced 3 full-time data analysts. The reports are more accurate, available instantly, and cost 70% less to produce.', name: 'M.W.*', title: 'CTO, Financial Services Firm', metric: '70% reporting cost cut' },
                { quote: 'We scaled from 50 to 500 daily transactions without hiring a single additional ops employee. CodeCrafters built the infrastructure that made that possible.', name: 'P.S.*', title: 'Founder, Retail Group', metric: '10x scale, same team size' },
                { quote: 'The OCR engine alone saves my team 3 hours a day on invoice entry. Confidence scoring means we only review the edge cases — everything else is automatic.', name: 'R.M.*', title: 'Finance Director, Manufacturing Co.', metric: '80% manual entry eliminated' },
                { quote: 'Migration from v14 to v19 with zero downtime and not a single data loss. I was expecting 6 months of pain. It took 8 weeks and was completely smooth.', name: 'A.T.*', title: 'CTO, eCommerce Group', metric: 'Zero downtime migration' },
                { quote: 'CodeCrafters reduced our order processing time from 4 hours to 8 minutes. The Odoo implementation paid for itself in the first quarter.', name: 'S.C.*', title: 'COO, Logistics Enterprise', metric: '96% faster processing' },
                { quote: 'Their AI pipeline replaced 3 full-time data analysts. The reports are more accurate, available instantly, and cost 70% less to produce.', name: 'M.W.*', title: 'CTO, Financial Services Firm', metric: '70% reporting cost cut' },
                { quote: 'We scaled from 50 to 500 daily transactions without hiring a single additional ops employee. CodeCrafters built the infrastructure that made that possible.', name: 'P.S.*', title: 'Founder, Retail Group', metric: '10x scale, same team size' },
                { quote: 'The OCR engine alone saves my team 3 hours a day on invoice entry. Confidence scoring means we only review the edge cases — everything else is automatic.', name: 'R.M.*', title: 'Finance Director, Manufacturing Co.', metric: '80% manual entry eliminated' },
                ...
                <div key={i} className="w-[300px] sm:w-[340px] shrink-0 p-6 bg-[#0e0e0e] border border-[#4e4633]/15 hover:border-[#f5c518]/30 transition-colors duration-300 flex flex-col">
                  <div className="flex gap-0.5 mb-4">
                    {[...Array(5)].map((_, j) => <span key={j} className="material-symbols-outlined text-primary-container text-sm" style={{ fontVariationSettings: "'FILL' 1" }}>star</span>)}
                  </div>
                  <p className="text-on-surface-variant text-xs leading-relaxed flex-grow italic mb-5">"{t.quote}"</p>
                  <div className="border-t border-[#4e4633]/20 pt-4 flex items-end justify-between">
                    <div>
                      <p className="text-on-surface font-bold text-xs">{t.name}</p>
                      <p className="text-[#4e4633] text-[10px]">{t.title}</p>
                    </div>
                    <span className="text-primary-container text-[10px] font-black uppercase tracking-widest">{t.metric}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <p className="text-[#4e4633] text-[10px] text-center mt-6 px-5">* Names changed for client confidentiality</p>
        </section>
